...
C     estimator of the standardized spatial Moran's I-statistics. 
C

      subroutine Imrcoreinh(x,y,txy,n,s,ns,slambda,ks,hs,wrs,wts,
     +     wbi,wbimod,wss,edg,mumr,Imr)
     
      implicit real*8(a-h,o-z)

      integer i,j,iu,n,ns,ks,edg
      double precision inhwij,inhvij,hs,kerns,Imrminh,Imrninh,Imr
      double precision hij,mij,xi,yi,ti,two,wrs,wts,wbi,x,y,txy
      double precision wbimod,wss,slambda,mumr
      dimension x(n),y(n),txy(n),s(ns),Imrminh(ns),Imrninh(ns)
      dimension wrs(n,n),wts(n,n),wbi(n,ns),wbimod(n,ns),wss(ns)
      dimension ks(3),edg(6),slambda(n),Imr(ns)
...
C    none
      if (edg(1).eq.1) then
       ...
       Imrminh(iu)=Imrminh(iu)+inhwij
       Imrninh(iu)=Imrninh(iu)+inhvij  
      end if                  
C    isotropic
      if (edg(2).eq.1) then                  
