...
C     estimator of the standardized temporal Moran's I-statistics. 
C

      subroutine Imtcoreinh(snorm,txy,n,t,nt,tlambda,kt,ht,wrt,wtt,
     +     wbit,wbimodt,wst,edg,mumt,Imt)
     
      implicit real*8(a-h,o-z)

      integer i,j,iv,n,nt,kt,edg
      double precision inhwij,inhvij,ht,kernt,Imtminh,Imtninh,Imt
      double precision tij,mij,snormi,ti,wrt,wtt,wbit,txy,snorm
      double precision wbimodt,wst,tlambda,mumt
      dimension snorm(n),txy(n),t(nt),Imtminh(nt),Imtninh(nt)
      dimension wrt(n,n),wtt(n,n),wbit(n,nt),wbimodt(n,nt),wst(nt)
      dimension kt(3),edg(6),tlambda(n),Imt(nt)
...
C    none
      if (edg(1).eq.1) then
       ...
       Imtminh(iv)=Imtminh(iv)+inhwij
       Imtninh(iv)=Imtninh(iv)+inhvij  
      end if                  
C    isotropic
      if (edg(2).eq.1) then                  
