Vmr <- function(xyt,s.region,s.lambda,ds,ks="epanech",hs,correction="none",approach="simplified"){
  ...
  vmrtheo <- ((bsupt-binft)^2)/12
  npt <- pxy$n[1]
  nds <- length(ds)
  emr <- Emr(xyt,s.region,s.lambda,ds,ks,hs,correction,approach)$eEmr
  eVmr <- rep(0,nds)
  
  storage.mode(eVmr) <- "double"
  
  if (appro2[1]==1){
    Vmrout <- .Fortran("Vmrcore",as.double(ptsx),as.double(ptsy),as.double(ptst),
                       as.integer(npt),as.double(ds),as.integer(nds),as.integer(ker2)
                       ,as.double(hs),as.double(emr),(eVmr),PACKAGE="msfstpp")
    
    eVmr <- Vmrout[[10]]
    
    invisible(return(list(eVmr=eVmr,ds=ds,kernel=kernel,vmrtheo=vmrtheo)))
  } else {
    ...
    Vmrout <- .Fortran("Vmrcoreinh",as.double(ptsx),as.double(ptsy),as.double(ptst),
    ...
    invisible(return(list(eVmr=eVmr,ds=ds,kernel=kernel,vmrtheo=vmrtheo,s.lambda=s.lambda)))
  }
}