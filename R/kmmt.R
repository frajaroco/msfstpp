kmmt <- function(xyt,t.region,t.lambda,dt,kt="epanech",ht,correction="none",approach="simplified"){
  ...
  if (missing(ht)){
    d <- dist(xyt[,3])
    ht <- dpik(d,kernel=ks,range.x=c(min(d),max(d)))
  }
  ...
  if (missing(dt)) {
    maxt <- (bsupt-binft)/4
    times <- make.even.breaks(maxt,npos=20)$r
  }
  if(dt[1]==0){dt <- dt[-1]
  }
  ...
  npt <- length(ptsx)
  ndt <- length(dt)
  snorm <- apply(pts,MARGIN=1,FUN=norm,'2')
  mummt <- mean(snorm)
  ekmmt <- rep(0,ndt)
  ...
  if (appro2[1]==1){
    kmmtout <- .Fortran("kmmtcore",as.double(snorm),as.double(ptst),as.integer(npt),as.double(dt),
                        as.integer(ndt),as.integer(ker2),as.double(ht),as.double(mummt),(ekmmt)
                        ,PACKAGE="msfstpp")
    ...
    if(any(correction=="border")|any(correction=="modified.border")){
      ...
      for(i in 1:nds) { 
        wbit[,i] <- (bi>dt[i])/sum((bi>dt[i])/t.lambda)
        wbimodt[,i] <- (bi>dt[i])/.eroded.areat(t.region,dt[j])
      }
      wbit[is.na(wbit)] <- 0
    }
    ...
    if(correction=="setcovf"){
      wsett <- tsetcovf(dt,ndt,longit)
      wst <- 1/wsett}
    
    kmmrout <- .Fortran("kmmtcoreinh",as.double(snorm),as.double(ptst),
                        as.integer(npt),as.double(dt),as.integer(ndt),as.double(t.lambda),
                        as.integer(ker2),as.double(ht),as.double(wrt),as.double(wtt),
