It <- function(xyt,t.region,t.lambda,dt,kt="epanech",ht,correction="none",approach="simplified"){
  ...
  storage.mode(eImt) <- "double"
  
  if (appro2[1]==1){
    Imtout <- .Fortran("Imtcore",as.double(snorm),as.double(ptst),as.integer(npt),as.double(dt),
                       as.integer(ndt),as.integer(ker2),as.double(ht),as.double(mumt),(eImt),
                       PACKAGE="msfstpp")
    ...
    Imtout <- .Fortran("Imtcoreinh",as.double(snorm),as.double(ptst),as.integer(npt),
                       as.double(dt),as.integer(ndt),as.double(t.lambda),as.integer(ker2),
                       as.double(ht),as.double(wrt),as.double(wtt),as.double(wbit),
                       as.double(wbimodt),as.double(wst),as.integer(correc2),as.double(mumt),
